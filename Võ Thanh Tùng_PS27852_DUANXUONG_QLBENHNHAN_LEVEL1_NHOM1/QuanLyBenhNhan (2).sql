...
-- View cung cấp thông tin về bệnh nhân kèm theo thông tin về bệnh án tương ứng
CREATE OR ALTER VIEW BenhNhan_BenhAn_View
AS
SELECT bn.MaBN, bn.HoTen AS TenBenhNhan, bn.DiaChi AS DiaChiBenhNhan,
       ba.MaBenhAn, ba.ChuanDoan, ba.TrieuChung, ba.PhuongPhapDieuTri,
...
-- View cung cấp thông tin về bệnh nhân, bệnh án của họ, cũng như thông tin về bảo hiểm y tế nếu có
CREATE OR ALTER VIEW BenhNhan_BenhAn_BaoHiemYTe_View
AS
SELECT bn.MaBN, bn.HoTen AS TenBenhNhan, bn.DiaChi AS DiaChiBenhNhan,
       ba.MaBenhAn, ba.ChuanDoan, ba.TrieuChung, ba.PhuongPhapDieuTri,
       ba.NgayNhapVien, ba.NgayXuatVien, ba.KetQua,
       byt.MaBH, byt.NgayBatDau, byt.NgayKetThuc
FROM BenhNhan bn
JOIN BenhAn ba ON bn.MaBN = ba.MaBN
LEFT JOIN BaoHiemYTe byt ON bn.MaBH = byt.MaBH;

SELECT * FROM BenhNhan_BenhAn_BaoHiemYTe_View;

-- View cung cấp thông tin về bệnh nhân, chi tiết phiếu khám bệnh của họ và thông tin về bác sĩ thực hiện khám
...
--JOIN BacSi bs ON pkb.MaBS = bs.MaBS;
