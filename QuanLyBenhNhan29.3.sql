...
UPDATE BaoHiemYTe
SET NgayBatDau = '2023-06-01', NgayKetThuc = '2024-05-31'
WHERE MaBH = 'BH67892';
go

UPDATE BaoHiemYTe
SET NgayBatDau = '2024-03-15', NgayKetThuc = '2025-03-14'
WHERE MaBH = 'BH67893';
go
...
('Diarrhea', 10000, 150, N'Thuốc tiêu chảy', N'Viên'),
('Ibuprofen ', 4000, 80, N'Thuốc nhức đầu', N'Viên'),
('Loratadine ', 3000, 60, N'Thuốc sổ mũi', N'Viên'),
('Cetirizine ', 7000, 60, N'Thuốc giảm dị ứng', N'Viên'),
('Omeprazole ', 8000, 90, N'Thuốc kháng viêm', N'Viên'),
('Metformin ', 70000, 100, N'Thuốc bổ máu', N'Viên');
go
...
(2, 1, 7, N'Dịch mũi có màu kỳ lạ', N'Dương tính với Covid', N'Cách ly bệnh nhân'),
...
(3, 2, 1, 3, 20, 3, N'1 viên mỗi lần, 1 lần/ngày', '2024-03-20'),
(1, 1, 2, 7, 20, 3, N'1 viên mỗi lần, 3 lần/ngày', '2024-03-20'),
(7, 1, 2, 7, 20, 3, N'1 viên mỗi lần, 3 lần/ngày', '2024-03-20'),
(8, 1, 2, 7, 20, 3, N'1 viên mỗi lần, 3 lần/ngày', '2024-03-20'),
(6, 2, 1, 3, 20, 3, N'1 viên sau bữa ăn, 3 lần/ngày', '2024-03-20'),
(8, 2, 1, 3, 20, 3, N'2 viên sau bữa sáng, 1 lần/ngày', '2024-03-20'),
(2, 4, 2, 4, 30, 2, N'1 viên mỗi lần, 2 lần/ngày', '2024-03-21'),
(5, 4, 2, 4, 30, 2, N'1 viên mỗi tối, 1 lần/ngày', '2024-03-20'),
(9, 4, 2, 4, 30, 2, N'2 viên sau bữa sáng, 1 lần/ngày', '2024-03-20'),
(10, 4, 2, 4, 30, 2, N'1 viên mỗi lần, 2 lần/ngày', '2024-03-21'),
(1, 3, 4, 1, 10, 1,N'2 viên sau bữa tối, 1 lần/ngày', '2024-03-22'),
(2, 3, 4, 1, 10, 3,N'1 viên mỗi lần, 1 lần/ngày', '2024-03-20'),
(3, 3, 4, 1, 10, 3, N'1 viên sau bữa ăn, 3 lần/ngày', '2024-03-20'),
(4, 3, 4, 1, 10, 3, N'2 viên sau bữa sáng, 1 lần/ngày', '2024-03-20');
go

-- Thêm dữ liệu vào bảng HoaDon 1213
...
go
--trigger báo bảo hiểm y tế hết hạn
create or alter trigger tger_BHYT
on BaoHiemYTe
for insert
as
	begin
	if(select NgayKetThuc from inserted) < GETDATE()
		begin
			print N'Bảo hiểm y tế hết hạn'				
			rollback transaction			
		end
	end
go
...
go
-- cac view 
-- View để hiển thị thông tin của bệnh nhân và bệnh án
CREATE OR ALTER VIEW BenhNhan_BenhAn_View
AS
SELECT bn.MaBN, bn.HoTen AS TenBenhNhan, bn.DiaChi AS DiaChiBenhNhan,
       ba.MaBenhAn, ba.ChuanDoan, ba.TrieuChung, ba.PhuongPhapDieuTri,
       ba.NgayNhapVien, ba.NgayXuatVien, ba.KetQua
FROM BenhNhan bn
JOIN BenhAn ba ON bn.MaBN = ba.MaBN;
GO

SELECT * FROM BenhNhan_benhAn_View;

CREATE OR ALTER VIEW BenhNhan_PhieuKhamBenh_BacSi_View
AS
SELECT bn.MaBN, bn.HoTen AS TenBenhNhan, bn.DiaChi AS DiaChiBenhNhan,
       pkb.MaBenhAn, pkb.TrieuChung, pkb.NgayKham,
       bs.HoTenBS, bs.DiaChi AS DiaChiBS, bs.SoDienthoai, bs.ChuyenMon, bs.ChucVu
FROM BenhNhan bn
JOIN BenhAn ba ON bn.MaBN = ba.MaBN
JOIN PhieuKhamBenh pkb ON ba.MaBenhAn = pkb.MaBenhAn
join ChiTietPK ctpk on ctpk.MaBenhAn = pkb.MaBenhAn
JOIN BacSi bs ON bs.MaBS = ctpk.MaBS;
go

select * from BenhNhan_PhieuKhamBenh_BacSi_View
go

-- View để hiển thị thông tin về các phiếu khám bệnh kèm theo thông tin của bác sĩ
CREATE OR ALTER VIEW PhieuKhamBenh_BacSi_View
AS
SELECT pkb.MaBenhAn, pkb.TrieuChung, pkb.NgayKham,
       bs.HoTenBS, bs.DiaChi AS DiaChiBS, bs.SoDienthoai, bs.ChuyenMon, bs.ChucVu
FROM PhieuKhamBenh pkb
JOIN BacSi bs ON pkb.MaBenhAn = bs.MaBS;
GO
select * from PhieuKhamBenh_BacSi_View

-- function 
-- Đếm số ca bệnh có triệu chứng được truyền vào
go
create function SoCaBenh(@TenBenh nvarchar(100))
returns int
as
begin
    declare @SoCa INT;
    ...
    select @SoCa = COUNT(*)
    from BenhAn
    where ChuanDoan like '%' + @TenBenh + '%';

    return @SoCa;
end;
go
select dbo.SoCaBenh(N'Đau răng') as SoCaDauRang;
go